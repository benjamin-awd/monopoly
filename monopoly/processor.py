...
from monopoly.pdf import PdfParser, PdfConfig
...
class StatementProcessor(PdfParser):
    def __init__(self, statement_config, file_path, pdf_config = None):
        ...
        self.pdf_parser: PdfParser = PdfParser(file_path, pdf_config)

        super().__init__(file_path=file_path, config=pdf_config)

    def extract(self) -> Statement:
        pages = self.pdf_parser.get_pages()
